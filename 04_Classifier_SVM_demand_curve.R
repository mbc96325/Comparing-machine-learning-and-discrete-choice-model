...
col_cost = 'dur_pt_access' # 'cost_driving_fuel' # dur_pt_bus cost_driving_ccharge
num_points = 100
...
for (DATASET in DATASET_list){
  for (Dependent_var in Dependent_var_list){
    ...
    for (sample_size in sample_size_list){
      if (sample_size != '1k'){
        print('SVM_R only works for 1k samples')
        next
      }
      if (DATASET == 'SG'){
        if (sample_size == '100k'){
      ...
      if (DATASET == 'London'){
        ...
        data_name_read = paste0('train_data_London',tail_name,'.csv')
        data_test_read = paste0('test_data_London',tail_name,'.csv')
