...
def get_data_info():
    ...
    res[('SG','MODE')] = {'input_dim':input_dim, 'num_alt':num_alt,'task':'TM','location':'AS'}
    ##
    ...
    res[('London','MODE')] = {'input_dim':input_dim, 'num_alt':num_alt,'task':'TM','location':'EU'}
    ##
    ...
    res[('NHTS','CAR_OWN')] = {'input_dim':input_dim, 'num_alt':num_alt,'task':'AO','location':'US'}

    ##
    data = pd.read_csv('data/data_mode_choice_1k.csv')
    input_dim = len(set(data.columns).difference({'MODE'}))
    num_alt = len(set(data['MODE']))
    res[('NHTS','MODE')] = {'input_dim':input_dim, 'num_alt':num_alt,'task':'TM','location':'US'}
    ...
    res[('NHTS','TRIPPURP')] = {'input_dim':input_dim, 'num_alt':num_alt,'task':'TP','location':'US'}
...
def process_exp_data():
    ...
    res = get_data_info()
    key_for_direct_info = ['input_dim','num_alt','task','location']
    key_for_dummy = ['task','location']
    for key in res:
...
def process_data_to_pair(used_data, sample_rate, sample_num):

    print('total models', len(used_data))
    ...
    used_data_cross = used_data_cross.loc[used_data_cross['model_id_x'] < used_data_cross['model_id_y']].reset_index(drop = True)
...
def estimate(data, all_tasks, all_location, all_model_family, save_name):
    ...
    data['sample_size_x'] /= 100000 # scale for fast convergence
    data['sample_size_y'] /= 100000
...
if __name__ == '__main__':
    data = pd.read_csv("../literature_review/literature_review.csv")
    ...
    if DCM:
        ESTIMATE = True
        sample_size = 2000
        save_name = f'meta_analysis_literature_data_{sample_size}'
        if ESTIMATE:
    ...
    if LR:
        ESTIMATE = True
        save_name = f'meta_analysis_literature_data_LR'
        if ESTIMATE:
