...
def warn(*args, **kwargs):
    pass
import warnings
warnings.warn = warn # avoid the warnings!
...
def DNN_estimate(x_train_nn_all, data_test, col_cost, num_points, MODE_WANT, choice_train_nn_all, cv, base, output_file_path, DNN_para, Re_run, computer_info,verbose_or_not, training_epochs,
                 Estimator_name, n_jobs):
    ...
    min_choice = min(choice_train_nn)
    if min_choice == 1:
        ...
        train_y = tf.keras.utils.to_categorical(choice_train_nn, num_classes=numAlts)
        test_y = tf.keras.utils.to_categorical(choice_test_nn, num_classes=numAlts)
    ...
    for point_value in all_points:
        ...
        y_pred_prob = model.predict(x_temp)
    ...
    Results = Results.append(
        pd.DataFrame({'Model': [name] * num_points, 'x_values': all_points, 'pred_prop': prop_list}))
    ...
    #
    # nn_train_acc = results_train[1]
    # print ('training_acc:', nn_train_acc)
    # correct_test = results[1]
    # accuracy.append(np.max(results[1]))
    # print('test_acc:', np.max(correct_test))
    ...
    for DATASET in DATASET_list:
        for Dependent_var in Dependent_var_list:
            ...
            for dnn_struc in dnn_struc_list:
                ...
                Estimator_name = 'DNN' + '_' + str(numUnits) + '_' + str(numHlayers) + '_python'
                ...
                for sample_size in sample_size_list:
                    ...
                    output_file_path = output_file_path.replace('.csv','_' + col_cost + '_' + 'mode' + str(int(MODE_WANT)) + '.csv')

                    num_cross_validation = 5
                    choice_train_nn_all = np.array(data.iloc[:, 0])
                    DNN_estimate(x_train_nn_all, data_test, col_cost, num_points, MODE_WANT, choice_train_nn_all, num_cross_validation, base, output_file_path, DNN_para, Re_run, computer_info, verbose_or_not,training_epochs,
