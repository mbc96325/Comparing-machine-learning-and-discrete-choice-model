...
def model_run(raw_data, data_test_raw, col_cost, num_points, MODE_WANT, output_file_path, cv, Re_run, computer_info, model_name, n_jobs, Dependent_var, DATASET, Only_prob_est):
    ...
    walk_var = ['dur_walking']
    cycle_var = ['dur_cycling']
    pt_var = ['dur_pt_access','dur_pt_rail','dur_pt_bus','dur_pt_int','pt_interchanges','cost_transit']
    drive_var = ['dur_driving','cost_driving_fuel','cost_driving_ccharge','driving_traffic_percent']

    specification['MODE_1'] += ['B___' + name + '___' + 'MODE_1' for name in walk_var]
    specification['MODE_2'] += ['B___' + name + '___' + 'MODE_2' for name in cycle_var]
    specification['MODE_3'] += ['B___' + name + '___' + 'MODE_3' for name in pt_var]
    specification['MODE_4'] += ['B___' + name + '___' + 'MODE_4' for name in drive_var]

    #
    #print(x_columns)
    x_columns_share = set(x_columns).difference(set(walk_var + cycle_var + pt_var + drive_var))

    for name in x_columns_share:
        initial_seed += 1
    ...
    html_name_cv = html_name_cv.replace('_' + col_cost + '_' + 'mode' + str(int(MODE_WANT)),'') # whatever col_cost and Want is, the estimation model is the same.
...
if __name__ == '__main__':
    ...
    Estimator_name = 'NL_biogeme_ASUSpec'
    # DATASET_list = ['NHTS', 'London', 'SG']
    DATASET_list = ['London']

    Only_prob_est = True
    col_cost = 'cost_driving_ccharge' # 'cost_driving_fuel' # cost_driving_ccharge dur_pt_access
    num_points = 100
    ...
    for DATASET in DATASET_list:
        for Dependent_var in Dependent_var_list:
            ...
            for sample_size in sample_size_list:
                ...
                model_run(data, data_test, col_cost, num_points, MODE_WANT, output_file_path, 5, Re_run, computer_info, Estimator_name, n_jobs, Dependent_var, DATASET, Only_prob_est)
